...
#[Route('/tache')]
class AddTacheController extends AbstractController
{
            ...
            if (false === $this->authorizationChecker->isGranted('ROLE_ADMIN')) {
                ...
                if ($form->isSubmitted() && $form->isValid()) {
                    ...
                    if ($programmeExistant != null) {
                        $this->addFlash('error', 'Une tache ayant le même libellé ' . $programmeExistant->getLibelleActivite() . ' existe déjà.',
                        );
                        return $this->redirectToRoute('app_tache_new', array('id' => $activite->getId()));
                    }
...
//                    }
                    ...
                    //Mouchard
                    $this->saveMouchard("tache", $activite->getId(), $activite->getLibelleActivite(),
                        "enregistrement", "profil", "user", "");

                    //Flashbag
                    $this->addFlash('success', 'Enregistrement de la tache '. $activite->getLibelleActivite() .' effectué avec succès.');
